...
function buildScatterSVG(args: {
  ...
  const { rows1, rows2, username, username2, xChoice, yChoice, avgMetric, xMax, yMax } = args
  const W = 800
  const H = 420
  ...
  const unitXTicks =
    xKey === "Total_Posts" && capX <= 15 ? Array.from({ length: Math.max(0, Math.floor(capX)) }, (_, i) => i + 1) : null
    ...
    .map(
      (v) =>
        `<line x1="${sx(v)}" y1="${PAD.t + innerH}" x2="${sx(v)}" y2="${PAD.t + innerH + 4}" stroke="#9ca3af"/>
         <text x="${sx(v)}" y="${PAD.t + innerH + 16}" text-anchor="middle" font-size="10" fill="#6b7280">${fmt.format(
           v
         )}</text>`
    )
    .join("")
  ...
  const s1 = fa1
    .map(
      (d) =>
        `<circle cx="${sx(Number((d as any)[xKey]))}" cy="${sy(Number((d as any)[yKey]))}" r="${sr(
          d.Subreddit_Subscribers
        )}" fill="var(--c1, #3b82f6)" fill-opacity="0.7"><title>${d.sub}</title></circle>`
    )
    .join("")
  const s2 = fa2
    .map(
      (d) =>
        `<circle cx="${sx(Number((d as any)[xKey]))}" cy="${sy(Number((d as any)[yKey]))}" r="${sr(
          d.Subreddit_Subscribers
        )}" fill="var(--c2, #14b8a6)" fill-opacity="0.7"><title>${d.sub}</title></circle>`
    )
    .join("")
  ...
  return `
    <svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Scatter">
      <style>
...
function buildBarSVG(args: {
  ...
  const { rows, label, metric = "Avg_Upvotes_Per_Post", xLabelOffset = 34, maxBars = 25 } = args
  ...
  const xTicks = ticks
    .map(
      (v) => `
        <line x1="${sx(v)}" y1="${PAD.t + innerH}" x2="${sx(v)}" y2="${PAD.t + innerH + 4}" stroke="#9ca3af"/>
        <text x="${sx(v)}" y="${PAD.t + innerH + 16}" font-size="10" fill="#6b7280" text-anchor="middle">${fmt.format(
          v
        )}</text>
      `
    )
  ...
  return `
    <svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Bar">
      <style>
        text { font-family: ui-sans-serif, -apple-system, Segoe UI, Roboto, Arial, sans-serif; }
      </style>
      ${axis}
...
function summarizeSeries(
  ...
  keys: string[],
  how: "avg_upvotes" | "avg_comments" | "total_upvotes"
) {
  ...
  for (const r of base) {
    ...
    else val = nums.length ? nums.reduce((a, b) => a + b, 0) / nums.length : null
...
function aggregateSeries(
  series: Array<{ date: string; v: number | null }>,
...
function buildLineSVG(args: {
  ...
  const { ts1, ts2, username, username2, metric, granularity, xLabel, yLabel } = args
  if (!ts1) return ""
  ...
  const legend = `
    <g>
      ...
      ${
        ts2
          ? `<rect x="${W - 220}" y="${PAD.t + 16}" width="12" height="2" fill="#14b8a6"/>
             <text x="${W - 204}" y="${PAD.t + 20}" font-size="12" fill="#374151">${`u/${username2 || "user2"}`}</text>`
          : ""
      }
    </g>
  `
  ...
  return `
    <svg viewBox="0 0 ${W} ${H}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Line">
      <style>
        text { font-family: ui-sans-serif, -apple-system, Segoe UI, Roboto, Arial, sans-serif; }
      </style>
      ${axis}
...
function buildInsightsHTML(insights: string[] | null | undefined) {
...
function normalizeUsers(body: any): Array<{ username: string; rows: any[]; timeSeries: TS | null; cqs: string | null }> {
  const fromUsers = Array.isArray(body?.users)
    ? body.users
        .map((u: any) => ({
          username: String(u?.username || "").trim(),
          rows: Array.isArray(u?.rows) ? u.rows : [],
          timeSeries: (u?.timeSeries ?? null) as TS | null,
          cqs: typeof u?.cqs === "string" ? u.cqs : null,
        }))
        .filter((u: any) => u.username && u.rows.length > 0)
    : []

  if (fromUsers.length) return fromUsers.slice(0, 6)

  const legacy: Array<{ username: string; rows: any[]; timeSeries: TS | null; cqs: string | null }> = []
  const u1 = String(body?.username || "").trim()
  const r1 = Array.isArray(body?.rows) ? body.rows : []
  if (u1 && r1.length) legacy.push({ username: u1, rows: r1, timeSeries: (body?.timeSeries ?? null) as TS | null, cqs: body?.cqs ?? null })

  const u2 = String(body?.username2 || "").trim()
  const r2 = Array.isArray(body?.rows2) ? body.rows2 : []
  if (u2 && r2.length) legacy.push({ username: u2, rows: r2, timeSeries: (body?.timeSeries2 ?? null) as TS | null, cqs: body?.cqs2 ?? null })

  return legacy.slice(0, 6)
}

function buildKpiHTML(args: {
  users: Array<{ username: string; rows: any[]; cqs: string | null }>
  dateRange: string
  limit: number
  inclPER: boolean
}) {
  const { users, dateRange, limit, inclPER } = args
  ...
  return `
    <div class="kpi-stack">
      ${safe
        .map((u) => {
          const k = kpiCompute(u.rows || [], dateRange, limit, inclPER)
          const cqs = typeof u?.cqs === "string" && u.cqs.trim() ? u.cqs.trim() : "—"
          const uname = u.username || "User"
          return `
            <div class="kpi-user">
              <div class="kpi-user-title">u/${uname}</div>
              <div class="kpi-grid">
                <div class="kpi-card">
                  <div class="kpi-title">Total Posts Analyzed</div>
                  <div class="kpi-value">${k.totalPosts.toLocaleString()}</div>
                  <div class="kpi-note">${k.timeNote}</div>
                </div>

                <div class="kpi-card">
                  <div class="kpi-title">Overall Upvote Score</div>
                  <div class="kpi-value">${k.overallUpvotes.toLocaleString()}</div>
                  <div class="kpi-note">Sum of upvotes across analyzed posts</div>
                </div>

                <div class="kpi-card">
                  <div class="kpi-title">Contributor Quality Score</div>
                  <div class="kpi-value">${cqs}</div>
                  <div class="kpi-note">CQS rating</div>
                </div>

                <div class="kpi-card">
                  <div class="kpi-title">Top Subreddit</div>
                  <div class="kpi-value">${k.topSubreddit}</div>
                  <div class="kpi-note">Highest ${k.avgKeyLabel}</div>
                </div>

                <div class="kpi-card">
                  <div class="kpi-title">Biggest Opportunity</div>
                  <div class="kpi-value">${k.biggestOpportunity}</div>
...
export async function POST(req: NextRequest) {
  ...
  const username2 = secondary?.username || ""

  const rowsPrimary: any[] = Array.isArray(primary?.rows) ? primary!.rows : []
  const rowsSecondary: any[] = Array.isArray(secondary?.rows) ? secondary!.rows : []

  const ownR = username ? `r/${username.toLowerCase()}` : ""
  const ownU = username ? `u_${username.toLowerCase()}` : ""
  const excluded = new Set<string>()

  const filteredPrimary = rowsPrimary.filter((r: any) => {
  ...
  const css = `
    ...
    * { box-sizing: border-box; }
    ...
    .kpi-user { display: grid; gap: 8px; }
    .kpi-user-title { font-size: 12px; font-weight: 700; color: #111827; margin: 0; }
    ...
    .table-user-title { font-size: 12px; font-weight: 700; color: #111827; margin: 0 0 6px; }
  `
  ...
  if (flags?.inclPER) {
    tableCols.push({ key: "WPI_Score", label: "WPI Score" })
    tableCols.push({ key: "WPI_Rating", label: "WPI Rating" })
  }
  tableCols.push({ key: "LastDateTimeUTC", label: "Last Post (UTC)" })

  const tableHead = `<tr>${tableCols.map((c) => `<th>${c.label}</th>`).join("")}</tr>`

  function buildTableForUser(u: { username: string; rows: any[] }) {
    const uname = String(u?.username || "").trim()
    const baseRows = Array.isArray(u?.rows) ? u.rows : []

    const ownR2 = uname ? `r/${uname.toLowerCase()}` : ""
    const ownU2 = uname ? `u_${uname.toLowerCase()}` : ""
    const filtered = baseRows.filter((r: any) => {
      const sub = String(r?.Subreddit || "").toLowerCase()
      return sub !== ownR2 && sub !== ownU2
    })

    const n = Math.min(tableLimit, filtered.length)
    const top = pickTopN(filtered, n, "Total_Upvotes")
    ...
    return `
      <div class="table-user">
        <div class="table-user-title">u/${uname || "user"}</div>
        <div class="card">
          <table>
        ...
        <div class="note">Showing top ${top.length}${top.length < 25 ? " (reduced to fit)" : ""}. Columns reflect current selections.</div>
      </div>
    `
  }

  const limitForKpi = Number.isFinite(body?.limit) ? Number(body.limit) : 1000
  const kpiBlock = buildKpiHTML({
    users: users.map((u) => ({ username: u.username, rows: u.rows, cqs: u.cqs })),
    dateRange,
    limit: limitForKpi,
    inclPER: !!flags?.inclPER,
  })

  const tableBlock = `<div class="table-stack">${users.map((u) => buildTableForUser({ username: u.username, rows: u.rows })).join("")}</div>`
  ...
  const scatterBlock = buildScatterSVG({
    rows1: filteredPrimary,
    rows2: rowsSecondary,
    username,
    username2,
  ...
  const barBlock = buildBarSVG({
    rows: filteredPrimary,
  ...
  const ts1 = primary?.timeSeries ?? null
  const ts2 = secondary?.timeSeries ?? null

  const lineBlock = buildLineSVG({
    ts1,
    ts2,
    username,
    username2,
  ...
  const dynamicScatterTitle = `Subreddit Performance: ${labelFor(scatterYChoice, scatterAvgMetric)} vs. ${labelFor(
    scatterXChoice,
    scatterAvgMetric
  )}`

  const insightsBlock = buildInsightsHTML(insightsArr)
    ...
    <html>
      ...
      <body>
        <div class="section">
          ...
          <h1>${title}</h1>
          <div class="muted small">Date Range: ${dateRangeText} • Generated: ${now}${compareList ? ` • Compare vs ${compareList}` : ""}</div>
        </div>
